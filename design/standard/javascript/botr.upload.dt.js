...
function botr_add_upoload_progress(botr_active_at_id){
	...
	jQuery("#" + botr_active_at_id + " .uploadText").addClass('active');
	jQuery('#uploadForm').uploadProgress({
		...
		uploadToken: jQuery('#uploadToken').val(),
		interval:1000,
		complete: function(upload) {
//			jQuery("#" + botr_active_at_id + " .fileDetails").html("<p>You're video has been uploaded. Translation details will appear shortly.</p>");
		},
...
//			alert("Upload failed! Please discard this draft and try again!");
		...
		start: function() {
			...
			jQuery("#" + botr_active_at_id + " .uploadBar").css('display','block');
			jQuery("#" + botr_active_at_id + " .uploadButton").attr('disabled','disabled');
		},
		// During upload, we update both the progress div and the text below it.
		uploading: function(upload) {
			jQuery(".uploadText.active").html('Uploading ' + filename + ' (' + upload.percents + '%) ...');
		}
	});
}